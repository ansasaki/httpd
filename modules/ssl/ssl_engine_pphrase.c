...
int passphrase_ui_open(UI *ui);
int passphrase_ui_read(UI *ui, UI_STRING *uis);
int passphrase_ui_write(UI *ui, UI_STRING *uis);
int passphrase_ui_close(UI *ui);
UI_METHOD *get_passphrase_ui(server_rec *s, apr_pool_t *p, int idx,
                             const char *pkey_file,
                             apr_array_header_t **pphrases);
void destroy_passphrase_ui_method(UI_METHOD *ui_method);
#endif
...
int passphrase_ui_read(UI *ui, UI_STRING *uis)
{
    ...
    buf = (char *)OPENSSL_zalloc(bufsize);
    ...
    /*
     * Since the pass phrase have already been copied, the temporary buffer can be freed
     */
    OPENSSL_clear_free(buf, bufsize);
    return 1;
}
				                  ...
				                  EVP_PKEY **ppkey)
{
    ...
    if (mc->szCryptoDevice) {
        ...
        //TODO TAKE CARE of this later. Probably it would break in forking scenarios
        //TODO The functional reference to the engine is associated with the EVP_PKEY?
        if (ENGINE_init(e)) {
